def out2DB(templatename,pc,reaction_file,star_file,settings_file,atm_file_path='PHOTOCHEM/OUTPUT/out.atm'):
  
  if atm_file_path == 'PHOTOCHEM/OUTPUT/out.atm':
    test=searchDB(templatename,pc,reaction_file,star_file,settings_file,verbose=False)
    if (test=='identical'):
  ...
  pathtoDB='PHOTOCHEM/INPUTFILES/TEMPLATES/ATMDB/'  #hardcoding for ATMOS - needs flexibility for photochem
  ...
  if atm_file_path != 'PHOTOCHEM/OUTPUT/out.atm':
    username='Templates'  #assumption is that out2DB is being called from TestModels.py, which has already created a temporary out.yaml file in the root directory
    os.remove(pathtoDB+username+'Index.hdf5')
    os.remove(pathtoDB+username+'AtmDB.hdf5')

  
  

  indexfilename = pathtoDB+username+'Index.hdf5'
  DBfilename = pathtoDB+username+'AtmDB.hdf5'
  
  #check if DBfile has grown too large (capping at 500MB for now...)
  max_size=0.5*1024*1024*1024
  if os.path.isfile(DBfilename):
     if os.path.getsize(DBfilename) > max_size:
      print('Database file', DBfilename, 'is larger than', max_size/(1024*1024), 'MB. Renaming')
      from datetime import datetime
      now = datetime.now()
      new_DBfilename = pathtoDB+f"{username}AtmDB_{now.strftime('%m%d%Y')}.hdf5"
      os.rename(DBfilename, new_DBfilename)
      new_indexfilename = pathtoDB+f"{username}Index_{now.strftime('%m%d%Y')}.hdf5"
      os.rename(indexfilename, new_indexfilename)  #this will trigger the creation of a new index/atm file below

  
  x[0]=DBfilename  #add DB filename to index (with templatename being the name of the dataset in the DB)
  

  WholeDB=createVDS(varnames)
  if os.path.isfile(WholeDB):
    with h5py.File(WholeDB, "a") as f:
      nelements=f[varnames[0]].shape[0] #number of atmospheres currently in the eniter DB
      
      tnames=f['templatename']
      if templatename in [tn.decode() for tn in tnames]:
        print('An atmosphere named', templatename,'already exists in the DB. Not saving...')
        return 
  ...
  if os.path.isfile(indexfilename):
    ...
    print('Creating new index file', indexfilename)
    dTypes=['f8']*32
  ...
  #need to implement an iff here on 'pc == 'ATMOS' and then do a direct read in of the atm file... (check the write_atm_txt for hints)
  if pc == 'ATMOS':
    sol=read_atm_txt(atm_file_path)
  else: # assuming pc is from photochem
...
def searchDB(templatename,pc,reaction_file,star_file,settings_file,verbose=True):
    import numpy as np
    ...
    #k should scale with size of DB, but more importantly, needs to be large enough that k doesn't return only 0-distance cases in the index search
    #if it does that, it could miss the best case that matches the boundary conditions. so need to increase k inline until at least 1 non-0 distance case is found.


    with h5py.File(indexfilename, "r") as f:
      #create a (numvariables - 2, nummodels) array
      nummodels=f[varnames[0]].shape[0]
      if (verbose): print('Searching database of',nummodels,'atmospheres for nearest-neighbors to',templatename)
      k=min(20,nummodels-1) 
    ...
    initialdistances=np.sum(DB,axis=0) #this sums through the variables for each model
    #so initialdistances is now 1 1D array of dimension (nummodels)
    
    idx=np.argpartition(initialdistances,k)
    
    count = np.count_nonzero(initialdistances==0.0)
    #print (count,'  instances of 0')
    if count >=k:
      k=count+5
      if (verbose): print ('increasing k and trying again - new k',k)
      idx=np.argpartition(initialdistances,k)
     
    notes = np.full(k, 'unique', dtype='<U10') #assume unique unless otherwise noted below

    if (verbose): print(k,'Nearest-Neighbors to',templatename, 'are:')
    fnames=[]
    tnames=[]
    with h5py.File(indexfilename, "r") as f:
      for i in range(k):
        ...
        if (verbose): print('  ', '{:2.4e}'.format(initialdistances[idx[i]]),' - ', tnames[i] + ' which is saved in ' + fnames[i] )
        if(DB[4,idx[i]] != 0.0):
          if (verbose): print('     **WARNING - THIS FILE HAS A DIIFERENT NUMBER OF SPECIES SO REQUIRES TRANSFORMATION*****')
          notes[i]='warning'
        if(DB[9,idx[i]] != 0.0):
          if (verbose): print('     **WARNING - THIS FILE HAS A DIIFERENT NUMBER OF LAYERS SO REQUIRES TRANSFORMATION*****')
          notes[i]='warning'
    ...
    if (verbose): print('Now checking on boundary conditions...')
    ...
    for i in range(k):
      with h5py.File(fnames[i], "r") as f:
        ...
        if (verbose): print(' Penalty for',tnames[i],'is',checkBoundaryConditions(new_bcs,DB_bcs,verbose=False))
    ...
    if (verbose): print('Best fit (including boundary conditions) is',tnames[bestfitindex],'in',fnames[bestfitindex])
    if math.isclose(finaldistances[bestfitindex],0.0, abs_tol=1e-10):
      if (verbose): print(' Identical match to an atmosphere already in the database')
      note='identical'
      return(note)

    note=notes[bestfitindex]
    #would be more useful to return the best non-warning

    if note == 'warning':
      print('Best fit atmosphere is incompatible with current species/heights')
      print('It is time to consider writing an interpolator or add/remove species code to atmDB.py') 
      print('')
      print('For now, returning best fit atmosphere with identical species/heights, if it exists')   
      mask = notes != 'warning'
      indices=np.where(mask)[0]
      if len(indices)==0:
        print('No compatible atmospheres found in DB.')
        note='warning'
      else:
        finidx=np.where(finaldistances[mask] == finaldistances[mask].min())
        bestfitindex=indices[finidx[0][0]]
        print('Returning best fit compatible atmosphere, which is',tnames[bestfitindex],'in',fnames[bestfitindex])
        note='unique'
    ...
    with h5py.File(fnames[bestfitindex],'r') as f:
      ...
      write_atm_txt(header,data) #this writes best fit atmosphere to PHOTOCHEM/INPUT/in.atm
...
def write_atm_txt(header,data):
  ...
  with open('PHOTOCHEM/INPUTFILES/in.atm', 'wt', encoding="utf-8") as fnew: #this will overwrite
...
def read_atm_txt(filename):
    import numpy as np
    with open(filename, 'r') as f:
        # Read the header line (skip leading spaces, split by fixed width 27)
        header_line = f.readline()
        headers = [header_line[i:i+27].strip() for i in range(3, len(header_line), 27)]
        # Read the rest of the file as data
        data = []
        for line in f:
            if line.strip() == '':
                continue
            # Each row starts with 3 spaces, then columns of width 27
            row = [float(line[i:i+27]) for i in range(3, len(line), 27)]
            data.append(row)
        arr = np.array(data)
    # Build dictionary
    # Only keep as many headers as columns in arr
    headers = headers[:arr.shape[1]]
    out = {h: arr[:,i] for i, h in enumerate(headers)}
    return out
...
def checkBoundaryConditions(new_bcs,DB_bcs,verbose=False): #eventually over DBfilename and templatename...
  ...
  for i in range(len(new_bcs)):
    try:
      ...
      except KeyError: #either new is long-lived and DB is short-lived, which is a mismatch, or both are long-lived
        
        #test for DB short-lived
        try:
          DB_bcs[species_match_index]['type']
          Penalty +=1
          if verbose: print(new_species[i], 'is long-lived in new template but short-lived in DB. Penalty =',Penalty )
        except KeyError:  #ok, reaching here means both are long-lived, so check upper and lower BCs
          ...
          if(newLBCtype=='Moses'):
            ...
            #reach here if NOT a direct match between types.
              ...
              Penalty += 0.3
              if verbose: print(new_species[i], 'LBC is',newLBCtype,'and',DB_bcs[species_match_index],'is',DBLBCtype,' Penalty =',Penalty)
...
def createVDS(varnames):
  ...
  pathtoDB='PHOTOCHEM/INPUTFILES/TEMPLATES/ATMDB/'
  Indexfiles=glob.glob(os.path.join(pathtoDB,'*Index.hdf5')) # a list of index files to concatenate
  shapes=[h5py.File(file)[varnames[0]].shape[0] for file in Indexfiles] #get the shapes (aka number of models) in each dataset
  
  dTypes=['f8']*32
  dt = h5py.string_dtype(encoding='utf-8')
  dTypes[0]=dt ; dTypes[1]=dt ; dTypes[3]=dt ; dTypes[5]=dt
  Vname=os.path.join(pathtoDB,'VDS.h5')
  with h5py.File(Vname,"w",libver='latest') as f: #note this is set to 'w' which overwrites each time...
...
def mapinput(templatename,pc,reaction_file,star_file,settings_file):
    ...
    with open(settings_file,'r') as file:
      ...
      s=yaml.load(file, Loader=yaml.FullLoader) #need to do this to consistently read in extras properly

    if pc=='ATMOS':
      modname=pc
      __version__ = s['extras']['model-version']
      specnum=s['extras']['species']
      rxver=s['extras']['reactions-version']
      surftemp=s['extras']['surface-temperature']
      surfedd=s['extras']['surface-eddy']
     
      try:
        starname = int(star_file) #python args from command lines are strings
      except ValueError:
        starname = star_file
    else:
      modname='photochem'
      from photochem import __version__ 
      specnum=pc.dat.nsp #number of species in the photochem model
      with open(reaction_file,'r') as file:
        rx=yaml.safe_load(file)
      rxver=rx['version']
      surftemp=pc.var.temperature[0] #surface temperature in Kelvin
      surfedd=pc.var.edd[0] #surface eddy diffusion in cm^2/s
      starname=Path(star_file).name
      
    extras={'templatename': templatename, 'model': modname, 'model-version': __version__, 'species': specnum, 'reactions-file': Path(reaction_file).name, 'reactions-version': rxver, 'star': starname, 'surface-temperature': surftemp, 'surface-eddy': surfedd }


    x=['DBfilename.DB']  #(0) DB filename - overwritten prior to save
    x.append(templatename) #(1) - add templatename string 
    
    if extras['model'] == 'photochem': 
      normvar=0.0
    ...
    #reactions-file
    if extras['reactions-file'] == 'zahnle_earth.yaml': #photochem default
      normvar=0.0
    ...
    if extras['star'] =='Sun_now.txt':  #used in photochem
      normvar=0.0
    elif extras['star'] =='Sun_4.0Ga.txt': #used in photochem
      normvar=-0.2
    ...
    try:
      RH=float(s['planet']['water']['relative-humidity'])
    except ValueError:
      ...
      normvar=-2.0
    else:  #RH should be a float between 0 and 1 - can just map directly!
      normvar=RH
    ...
    try:
      ...
      normvar=-1.0
    else: #rainfall rate is between say 1e-9 and 1e2? -> log scaling min-9, max=2, mean = -3.5
    ...
    try:
      ...
      normvar=-1.0
    else: # in cm, should be say between 1e5 (1 km) and 1e7 (100km) - log 5,7 mean 6
    ...
    try:
      ...
      normvar=-1.0
    else: #lets range between 1e-10 and 1e-2 - log space this is -10,-2 mean -6
    ...
    if extras['model'] == 'photochem':  #no lightning in photochem
      ...
      PRONO=s['planet']['lightning']
      #default modern Earth is 1e9 - scale from say 1e0 to 1e10 in log space and map to [0,1+) min= 0, max = 10, mean = 5
      if PRONO !=0.0:
        normvar=math.log10(PRONO)/10 #this takes 1e-1 to less than 0, but that's fine...  
      else:
